...
@font-face {
    font-family: 'Roboto';
    src: url('/mapas/common/fonts/Roboto-Bold.ttf') format('truetype');
		    font-weight: bold;
}

@font-face {
    font-family: 'Roboto';
		    src: url('/mapas/common/fonts/Roboto-Regular.ttf') format('truetype');
		    font-weight: normal;
}
...
#selector-pobreza:hover {
	background-color: rgb(180,171,201);
}
...
#slider-box {
	...
	<body>
		...
		<?php include_once('/srv/www/cartografica.mx/public_html/mapas/common/analyticstracking.php'); ?>
		<img id="img_header" src="img/coneval-2.png"/>
		...
		<div class="varlist" id="varlist_pobreza">
    		<div class="light text datos-title">Variables
	    		<img style="float: right; margin-top: -5px;" height="30" width="48" src="img/personas.png" />
    		</div>
		</div>
		
		<div class="varlist" id="varlist_rezago_social" style="display: none;">
    		<div class="green-light green-text datos-title">Variables
	    		<img style="float: right; margin-top: -5px;" height="30" width="48" src="img/personasV.png" />
    		</div>
		</div>
			...
			<div class="mundata-box beneficiarios_box">
				...
				<div id="legend">
					<div id="legend-pobreza">
						<div style="background-color: #f2f0f7;" class="legend-box"></div>
						<div style="background-color: #cbc9e2;" class="legend-box"></div>
						<div style="background-color: #9e9ac8;" class="legend-box"></div>
						<div style="background-color: #756bb1;" class="legend-box"></div>
						<div style="background-color: #54278f;" class="legend-box"></div>
		...
		<script type="text/javascript" charset="utf-8">
			...
			function add_municipios_layer(v1,type) {
				...
				utfGrid.on('mouseover', function (e) {
					if (current_clavegeo != e.data.cve_inegi) {
						current_clavegeo = e.data.cve_inegi;
						$(".nom_mun").html(e.data.nom_mun);
						$(".state_name").html(entidades_list[parseInt(e.data.ent)]);
						$(".num_mun").html(entidades_count_mun[parseInt(e.data.ent)] + " municipios");
						$(".state_image").html("<img src='img/estados_"+current_color.replace("#","")+"/"+parseInt(e.data.ent)+".png' />");
						
						if (c_type == "pobreza") {
							$("#nom_var").html(pobreza_vars[c_var].desc);
							$("#porcentaje").html(Math.round(e.data[c_var]*100*100)/100 + "%");
							$("#personas").html(commaSeparateNumber(e.data[pobreza_vars[c_var].nom_per]));
							// Chart
							var cScale = d3.scale.linear().domain([0, 100]).range([0, 2 * Math.PI]);
							
							var pie_data = [[0,(Math.round(e.data[c_var]*100*100)/100),current_color], [(Math.round(e.data[c_var]*100*100)/100), 100, "#eee"]];
							$("#inversion_pie").empty();
							...
							$("#porcentaje").html(Math.round(e.data[c_var + c_year]*100)/100 + "%");
							// Chart
							var cScale = d3.scale.linear().domain([0, 100]).range([0, 2 * Math.PI]);
	
							var pie_data = [[0,(Math.round(e.data[c_var + c_year]*100)/100),current_color], [(Math.round(e.data[c_var + c_year]*100)/100), 100, "#eee"]];
							$("#inversion_pie").empty();
							var vis = d3.select("#inversion_pie");
							var arc = d3.svg.arc()
							.innerRadius(20)
				...
				utfGrid.on('mouseout', function (e) {
					$(".nom_mun").html("Elija un municipio");
					$(".state_name").html("");
					$(".num_mun").html("");
					$(".state_image").html("");
					$("#nom_var").html("");
		    ...
		    var utfGrid = new L.UtfGrid('/tiles/coneval/tiles.py/coneval/{z}/{x}/{y}.json', {
				useJsonP: false
			});
